...
struct MemoryArgs {
	/// Guest RAM size
	#[clap(short = 'm', long, default_value_t, env = "HERMIT_MEM")]
	memory_size: GuestMemorySize,

	/// No Transparent Hugepages
	///
	/// Prevent advising the kernel to enable Transparent Hugepages [THP] on the virtual RAM.
	///
	/// [THP]: https://www.kernel.org/doc/html/latest/admin-guide/mm/transhuge.html
	#[clap(long)]
	no_thp: bool,

	/// No Kernel Samepage Merging
	///
	/// Prevent advising the kernel to enable Kernel Samepage Merging [KSM] on the virtual RAM.
	///
...
struct CpuArgs {
	...
	#[clap(short, long, default_value_t, env = "HERMIT_CPUS")]
	cpu_count: CpuCount,
